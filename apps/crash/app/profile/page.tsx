...
export default function ProfilePage() {
  const { user, demoBalance, adjustDemoBalance, authToken, apiBaseUrl } = useStore()
  ...
  useEffect(() => {
    if (!user?.telegramId || !authToken) return

    let isMounted = true
    const fetchProfileData = async () => {
      setIsLoading(true)
      try {
        const [profileResult, gamesResult, depositsResult] = await Promise.allSettled([
          crashApiFetch<UserProfile | null>(
            `/profile?telegram_id=${user.telegramId}`,
            {},
            authToken,
            apiBaseUrl
          ),
          crashApiFetch<{ games?: Game[] }>(
            `/profile/games?telegram_id=${user.telegramId}`,
            {},
            authToken,
            apiBaseUrl
          ),
          crashApiFetch<{ deposits?: Deposit[] }>(
            `/profile/deposits?telegram_id=${user.telegramId}`,
            {},
            authToken,
            apiBaseUrl
          ),
        ...
        if (profileResult.status === "fulfilled" && profileResult.value) {
          ...
          setNickname(profileResult.value.username || "")
        }
        ...
        if (depositsResult.status === "fulfilled") {
          setDeposits(depositsResult.value?.deposits || [])
        }
      } catch (error) {
        if (isMounted) {
  ...
  }, [user?.telegramId, authToken, apiBaseUrl])
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      const result = await crashApiFetch<UserProfile>(
        "/profile/update",
        {
          method: "POST",
      ...
      alert("Ошибка обновления профиля. Попробуйте позже.")
    }
  }
      ...
      <Card className="bg-gray-800/90 border-gray-700 mb-6 shadow-xl rounded-2xl w-full max-w-2xl mx-auto sm:p-8 p-3">
        ...
        <CardContent className="space-y-4 text-white">
          ...
          <div className="space-y-4 text-sm text-gray-300">
            <p>
              Все пополнения и выводы совершаются внутри Telegram-бота. Crash Mini App
              использует тот же баланс коинов и бонусов; управлять им можно через
              кнопки бота.
            </p>
          </div>
        </CardContent>
